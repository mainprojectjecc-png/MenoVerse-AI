    ...
    <div className="min-h-screen bg-[#fff9e8] flex flex-col text-[#3F3D35] font-['Plus_Jakarta_Sans']">
      ...
      <main className="flex-1 px-6 py-4 max-w-[480px] mx-auto w-full flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 flex-1">
          ...
          <div>
            <label className="text-[15px] font-medium text-[#1e1c10] mb-2 block">Age</label>
            <div className="relative group">
              <span className="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-[#A8A596] text-[22px]">calendar_today</span>
          ...
          {error && <p className="text-center text-[14px] text-[#BC6C4D] font-medium -mb-2">{error}</p>}
          <div className="pt-6">
            <button disabled={loading} type="submit" className="w-full bg-[#535845] hover:bg-[#444937] text-white py-[18px] rounded-2xl text-[16px] font-semibold shadow-[0_8px_20px_-8px_rgba(83,88,69,0.5)] flex items-center justify-center gap-2 active:scale-[0.98] transition-all disabled:opacity-60">
            ...
            <p className="text-center text-[15px] mt-8">Already have an account? <Link to="/login" className="font-semibold text-[#1e1c10] hover:underline">Sign in</Link></p>
          </div>
        </form>
      </main>
    </div>
